from scipy.sparse import spdiags, lil_matrix, vstack
...
class Multilink(object):
    ...
    def kinematic_constraint_mat(self, theta):
        ...
        Returns
        -------
        Cxx : 2d numpy array N * N
            coefficients for x_dot, x-direction constraint.
        Cxtheta : 2d numpy array N * N
            coefficients for theta_dot
        Cyy : 2d numpy array N * N
            coefficients for y_dot
        Cytheta : 2d numpy array N * N
            coefficients for theta_dot
        ...
        # Generate Cxx
        Cxx_part1 = lil_matrix(zeros((1, N)))
        Cxx_part1[0, 0] = -1
        Cxx_part2 = lil_matrix(spdiags(ones(N-1), 0, N-1, N) + spdiags(-ones(N), 1, N-1, N))
        Cxx = vstack((Cxx_part1, Cxx_part2))

        # Generate Cxtheta
        Cxtheta = lil_matrix(spdiags(-(1 / N) * sin(theta), 0, N, N))

        # Generate Cyy using Cxx (they are identical)
        Cyy = Cxx

        #Generate Cytheta
        Cytheta = lil_matrix(spdiags((1 / N) * cos(theta), 0, N, N))
    ...
    def viscous_torque_mat(self, x, y, theta):
        ...
        ----------
        x : 1d array
            x-coordinates of the left ends of link 2 through N
            x-coordinate of the right end of link N
        y : 1d array
            y-coordinates of the left ends of link 2 through N
            y-coordinate of the right end of link N
        theta : 1d array
            theta of link 1 through N

        Returns
        -------
        Tx : 2d numpy array N by N
            coefficients for x_dot
        Ty : 2d numpy array N by N
            coefficients for y_dot
        Ttheta : 2d numpy array N by N
            coefficients for theta_dot
        """
        if len(x) != self.N or len(y) != self.N or len(theta) != self.N:
            raise ValueError('x/y/theta should be of length {}'.format(self.N))

        N = self.N
        Tx = lil_matrix((N, N))
        Ty = lil_matrix((N, N))
        Ttheta = lil_matrix((N, N))
        ...
        # Get first row for Tx and Ty (with respect to link 1)
    ...
    def viscous_torque_coef(self, x, y, theta, link_theta, link_xy, xi, yi):
        ...
        elif link_theta < 0 or link_theta > self.N:
            raise ValueError('0<= link_theta <= {} required, while link_num = {}.'.format(self.N, link_theta))
